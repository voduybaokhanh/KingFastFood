document.addEventListener('DOMContentLoaded', () => {
	...
	function register() {
		...
		btnRegister.addEventListener('click', () => {
			...
			var REpassword = document.getElementById("js-RG_RePassword");
	...
	function Handle_LogOut() {
		...
		isLogin.addEventListener("click", (event) => {
		...
		for (var i = 0; i < showPayment.length; i++) {
			if (showPayment[i].username == nameUser) {
				...
				if (showPayment[i].status == 'confirmed') {
					value = "Đã xác nhận";
					color = "green";
				} else if (showPayment[i].status == 'pending') {
					value = "Đang xử lí";
					color = "orange";
				} else if (showPayment[i].status == 'unconfirmed') {
					value = "Đã huỷ";
					color = "red";
				}
